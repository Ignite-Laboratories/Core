...
func From(m ...measurement.Measurement) Phrase {
	return Phrase{
...
// FromNamed creates a named Phrase of the provided measurements and name.
func FromNamed(name string, m ...measurement.Measurement) Phrase {
	return Phrase{
		Name: name,
		Data: m,
	}
...
func FromBits(name string, bits ...std.Bit) Phrase {
	return Phrase{
		Name: name,
		Data: []measurement.Measurement{measurement.From(bits...)},
	}
}
...
func (a Phrase) GetData() []measurement.Measurement {
	return a.Data
}

// GetAllBits returns a slice of the Phrase's individual bits.
func (a Phrase) GetAllBits() []std.Bit {
	out := make([]std.Bit, 0, a.BitWidth())
...
func (a Phrase) Append(bits ...std.Bit) Phrase {
	if len(a.Data) == 0 {
		a.Data = append(a.Data, measurement.From(bits...))
		return a
	}
...
func (a Phrase) AppendBytes(bytes ...byte) Phrase {
	if len(a.Data) == 0 {
		a.Data = append(a.Data, measurement.FromBytes(bytes...))
		return a
	}
...
func (a Phrase) AppendMeasurement(m ...measurement.Measurement) Phrase {
	a.Data = append(a.Data, m...)
	return a
}

// Prepend places the provided bits at the start of the Phrase.
func (a Phrase) Prepend(bits ...std.Bit) Phrase {
	if len(a.Data) == 0 {
		a.Data = append(a.Data, measurement.From(bits...))
		return a
	}
...
func (a Phrase) PrependMeasurement(m ...measurement.Measurement) Phrase {
...
func (a Phrase) Align(width ...int) Phrase {
	...
	out := make([]measurement.Measurement, 0, a.BitWidth())
	current := make([]std.Bit, 0, w)
	i := 0
	...
	for _, m := range a.Data {
		for _, b := range m.GetAllBits() {
			...
			if i == w {
				i = 0
				out = append(out, measurement.From(current...))
				current = make([]std.Bit, 0, w)
			}
		}
	}
...
func (a Phrase) BleedLastBit() (std.Bit, Phrase) {
	if a.BitWidth() == 0 {
...
//
// NOTE: This is a destructive operation to the phrase's encoding scheme and returns a Raw Phrase.
func (a Phrase) BleedFirstBit() (std.Bit, Phrase) {
	if a.BitWidth() == 0 {
...
func (a Phrase) Reverse() Phrase {
	reversed := make([]measurement.Measurement, len(a.Data))
	ii := 0
